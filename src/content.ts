...
async function copyCommentToClipboard(button: HTMLButtonElement, commentData: CommentData): Promise<void> {
  try {
...
function addCopyButtonToComment(commentElement: Element, isReview = false): void {
  ...
  if (isReview) {
    ...
    const reviewThread = commentElement.closest('.review-thread-component, .js-resolvable-timeline-thread-container');

    if (reviewThread) {
      // File path from link in thread header
      const fileLink = reviewThread.querySelector('a.text-mono, a[href*="/files/"]');
      if (fileLink) {
    ...
    if (!commentData.file) {
      const diffFile = commentElement.closest('.js-file, [data-path], .file');
      if (diffFile) {
        commentData.file = diffFile.getAttribute('data-path') ||
                          diffFile.getAttribute('data-file-path') ||
                          diffFile.querySelector('.file-info a')?.textContent?.trim() ||
                          diffFile.querySelector('[data-path]')?.getAttribute('data-path') || '';
      }
    }

    // Find line numbers and code from the diff table
    const diffTable = reviewThread?.querySelector('.diff-table') || commentElement.closest('table.diff-table');
    if (diffTable) {
  ...
  const commentActions = commentElement.querySelector('.timeline-comment-actions, .comment-actions');
  if (commentActions) {
    commentActions.appendChild(button);
  } else {
    // Fallback: prepend to comment header
    const header = commentElement.querySelector('.timeline-comment-header, .review-comment-contents');
  ...
  mutations.forEach((mutation) => {
    mutation.addedNodes.forEach((node) => {
      if (node.nodeType === 1) { // Element node
        ...
        if (element.matches?.('.timeline-comment') || element.matches?.('.review-comment')) {
          addCopyButtonToComment(element, element.matches('.review-comment'));
        } else if (element.querySelectorAll) {
          element.querySelectorAll('.timeline-comment').forEach(c => addCopyButtonToComment(c, false));
          element.querySelectorAll('.review-comment').forEach(c => addCopyButtonToComment(c, true));
        }
      }
    });
  });
